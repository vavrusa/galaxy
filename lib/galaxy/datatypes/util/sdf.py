#!/usr/bin/python
# from https://searchcode.com/codesearch/raw/19269206/
#
...
#---------------------------------------------------------------- 
        def recinfos(self):
                for i,r in enumerate(self.records):
                        lines =  r['Structure_as_text'].split("\n")
                        # print lines
                        try:
                                self.records[i]['id']   = lines[0]
                                self.records[i]['natm'] = int(lines[3].split()[0])
                        except:
                                return False
                return True
                        # sys.exit(0)
...
#---------------------------------------------------------------- 
        def getheaders(self):
                ...
                for r in self.records:
                        # print r['Structure_as_text']
                        self.ids.append(r['Structure_as_text'][0])
...
#---------------------------------------------------------------- 
        def getnatoms(self):
                ...
                for r in self.records:
                        # print r['Structure_as_text']
                        self.natms.append(r['Structure_as_text'][3].split()[0])
...
if __name__ == "__main__":
        x = SDF_file(sys.argv[1])
        print x.nrecords
        for rec in x.records:
                print rec['id'], rec['natm']
